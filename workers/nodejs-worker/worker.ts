...
submissionQueue.process('process-submission', async (job: Job<SubmissionJobData>) => {
  ...
  if (!['NODEJS', 'NODE'].includes(language)) {
    console.log(`[NodeJS Worker] Ignorando submission ${submissionId} (language=${language})`);
    return;
  }
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    try {
      const run = await executeContainer({ image: 'node:20-alpine', cmd: ['node', '/tmp/solution.js'], code: data.code || '', input: tc.input || '', limits: { timeout: 5000, memory: 256, cpus: 1 }, fileName: 'solution.js' });
      ...
      console.log(`[NodeJS Worker] TC ${i + 1}/${testCases.length}: ${status} (${run.timeMs}ms)`);
    } catch (err: any) {
      console.error('[NodeJS Worker] Error en TC:', err.message);
  ...
  try {
    ...
    console.log(`[NodeJS Worker] Submission ${submissionId} finalizado: ${finalStatus}`);
    return { submissionId, finalStatus, passedCount, totalTestCases: testCases.length, totalTimeMs: totalTime };
  } catch (e: any) {
    console.error('[NodeJS Worker] Error guardando resultados:', e.message);
...
submissionQueue.on('completed', (job: Job) => console.log(`[NodeJS Worker] Job ${job.id} completado`));
submissionQueue.on('failed', (job: Job, err: Error) => console.error(`[NodeJS Worker] Job ${job.id} falló:`, err.message));
process.on('SIGTERM', async () => { console.log('[NodeJS Worker] Cerrando...'); await submissionQueue.close(); process.exit(0); });