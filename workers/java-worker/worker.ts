...
submissionQueue.process('process-submission', async (job: Job<SubmissionJobData>) => {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    try {
      // 1. Compilar
      const compile = await executeContainer({ image: 'openjdk:21-slim', cmd: ['javac', '/tmp/Solution.java'], code: data.code || '', input: '', limits: { timeout: 5000, memory: 256, cpus: 1 }, fileName: 'Solution.java' });
      ...
      // 2. Ejecutar
      const run = await executeContainer({ image: 'openjdk:21-slim', cmd: ['java', '-cp', '/tmp', 'Solution'], input: tc.input || '', limits: { timeout: 5000, memory: 256, cpus: 1 }, skipWrite: true });
